...
void poseCallback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    // xmin = std::min(post_min(0,0),post_max(0,0));
    // xmax = std::max(post_min(0,0),post_max(0,0));
    // ymin = std::min(post_min(1,0),post_max(1,0));
    // ymax = std::max(post_min(1,0),post_max(1,0));
    // zmin = std::min(post_min(2,0),post_max(2,0));
    // zmax = std::max(post_min(2,0),post_max(2,0));

    // Eigen::Vector4f pre_center(0,0,2.375,1);
    // float length = 4.75;
    // float height = 4;
    // float width = 8;
    // Eigen::Vector4f post_center;

    // post_center = initialT*pre_center;

    // xmin = std::min(post_center(0,0),post_center(0,0));
    // xmax = std::max(post_center(0,0),post_center(0,0));
    // ymin = std::min(post_center(1,0),post_center(1,0));
    // ymax = std::max(post_center(1,0),post_center(1,0));
    // zmin = std::min(post_center(2,0),post_center(2,0));
    // zmax = std::max(post_center(2,0),post_center(2,0));
